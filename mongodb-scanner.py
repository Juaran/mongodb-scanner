...
@author: magicming
改: 
添加用户名认证登陆：空密码登录失败则读取user.txt、pass.txt
"""
                ...
                try:
                    ...
                    for user in user_file:
                        ...
                        for passwd in pass_file:
                            ...
                            try:
                                self.lock.acquire()
                                print 'try to connect ' + ip + ':' + str(self.port) + ' by %s:%s' % (user, passwd)
                                self.lock.release()

                                client = MongoClient("mongodb://%s:%s@%s:27017/" % (user, passwd, ip))
                                client.server_info()
                                client.database_names()
                                client.close()
                                ...
                                self.lock.acquire()
                                success_flag = True
                                print '########## ' + ip + ':' + str(self.port) + ' Connect with %s:%s Success!' % (user, passwd)
                                file = open(self.fileout, 'a')
                                file.write(ip + ':' + str(self.port) + '\t' + user + ':' + passwd + '\n')
                                file.close()
                                self.success_count += 1
                                self.lock.release()
                                break       # 停止遍历
...
if __name__=='__main__':
    ...
    mcount = 200
    fileout = 'result.txt'
    users = 'user.txt'          # 读取的用户名
    passwds = 'pass.txt'        # 读取的密码
